...
void				AnimationRaycasterGPU::ProcessInput		( const sw::input::MouseState& mouse, const sw::input::KeyboardState& keyboard )
{
	...
	else if( keyboard[ Keyboard::PhysicalKeys::KEY_6 ].IsKeyDownEvent() )
	{
		m_animVertexShader = m_resourceManager->LoadVertexShader( L"Shaders/AnimationRaycasterGPU/VoxelAnimation4VS.hlsl", "main" );
		m_animGeometryShader = nullptr;
		m_animPixelShader = m_resourceManager->LoadPixelShader( L"Shaders/AnimationRaycasterGPU/VoxelAnimation4PS.hlsl", "main" );
	}
